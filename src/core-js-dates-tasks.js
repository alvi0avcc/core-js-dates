...
function getTime(...date) {
  ...
  return `${addZero(myDate.getHours())}:${addZero(myDate.getMinutes())}:${addZero(myDate.getSeconds())}`;
}
...
function getNextFriday(date) {
  ...
  const dayIdx = result.getDay();
  let daysToAdd;
  if (dayIdx < 5) {
    daysToAdd = 5 - dayIdx;
  } else {
    daysToAdd = 5 + (7 - dayIdx);
  }
...
function isDateInPeriod(date, period) {
  ...
  return init >= startDate && init <= endDate;
}
...
function getWeekNumberByDate(date) {
  ...
  const firstJanuary = new Date(Date.UTC(myDate.getUTCFullYear(), 0, 1));

  return Math.ceil(((myDate - firstJanuary) / (24 * 60 * 60 * 1000) + 1) / 7);
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (startDate <= endDate) {
    ...
    for (let j = 0; j < countOffDays && startDate <= endDate; j += 1) {
      startDate.setDate(startDate.getDate() + 1);
    }
  }
